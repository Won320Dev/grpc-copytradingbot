...
export async function sendTx(
  ...
  console.log('pump fun swap:', simulateResult);
  ...
  const result = await sendBundle(versionedTx, signers[0], latestBlockHash, 1500000);
  ...
  if (result) {
    ...
    return { success: false, signature: txSignature };
  }
}
